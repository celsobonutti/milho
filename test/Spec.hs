...
generatePow base power =
    [i|(call-with-error-handler
...
propLet x y z = monadicIO $ do
    ...
    assert $ result == expected
  where
    expected = Number . fromIntegral $ x + y + z

generateObject :: [Integer] -> Text
generateObject numbers =
    [i|(do
                              (import std/list)
...
propObject numbers = monadicIO $ do
    ...
    assert $ result == expected
  where
    expected = Number . fromIntegral . length $ numbers

generateMlist :: Integer -> Integer -> Integer -> Text
generateMlist fstInitial fstFinal snd =
    [i|(do
                                            (import std/mlist)
...
propMlist fstInitial fstFinal snd = monadicIO $ do
    ...
    assert $ result == expected
  where
    expected = Number . fromIntegral $ fstFinal
...
propMap numbers = monadicIO $ do
    ...
    assert $ result == expected
  where
    expected = Pair . List $ fmap (Number . fromIntegral . (+ 1)) numbers
...
propFoldr numbers = monadicIO $ do
    ...
    assert $ result == expected
  where
    expected = Number . fromIntegral $ foldr (-) 0 numbers

generateFilter :: Integer -> [Integer] -> Text
generateFilter compared numbers =
    [i|(list:filter
                                        (fn (x) (> x #{compared}))
...
propIf (Fn2 f) x y = monadicIO $ do
    ...
    assert $ result == expected
  where
    expected = Number . fromIntegral $ if f x y then x else y

generateGuard :: Integer -> Integer -> Text
generateGuard clauseNumber body =
    [i|(call-with-error-handler
...
generateUserRaise first second =
    [i|(if (> #{first} #{second})
                                 "yayy"
...
generateScopedNestedImport value =
    [i|(do
